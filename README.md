...
Corresponding `.conllu` files are also available alongside the `datasets/*.cgel` files.
...
- `convertor/`: a simple Flask web interface for local testing in the browser (English text > automatic UD w/ Stanza > CGEL). The DepEdit conversion rules themselves now live in `packages/cgel-json-converter/`.
...
`datasets/*.cgel` is the four gold files at the top of `datasets/`
(`twitter.cgel`, `ewt.cgel`, `ewt-test_iaa50.cgel`, `ewt-test_pilot5.cgel`).
It does not recurse into `oneoff/` or `iaa/`.
